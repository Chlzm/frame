...
//添加隐藏域用于存储date
    ...
    ;(function(){
        var airCity = {
            ...
            horn:function () {
                ...
                $('body').undelegate('.cityfrom .title li','clcik');
    ...
    ;(function () {
        ...
        SearchTip.prototype.bindInput = function () {
            var _this = this;
            var resultHeight=$(_this.result).height()<=1?24:$(_this.result).height();
            var config = {
                    li: "<li>",
            ...
            $("input[type=text]", this.result).on("input", function () {
                ...
                if(that.attr('name') == 'AirEndCity'){
                    var inputPosLeft=that.position().left;
                    $("ul.result", _this.result).css({"left":inputPosLeft,"z-index":"1000"});
                }
                else{
        ...
        SearchTip.prototype.confirm = function (that) {
            ...
            var _this = this,
                dom = $("ul.result>li.active", this.result);
            if (dom.index() != -1) {
                var inputClass;
    ...
    search.start({
        result: ".search-results",
        ajaxUrl:config._CONFIG_.airCityJson,
